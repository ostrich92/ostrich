...
if (nexacro.Browser == "Runtime") {
	if (!nexacro.Init_systembase_runtime) {
		...
		nexacro._download = function (url, handle, initname, targetpath, activemode) {
			var httpheaderstr = "";
			...
			var handle = nexacro.__downloadFileDownloadHandle(url, handle, initname, targetpath, httpheaderstr, activemode);
			if (handle == null) {
		...
		nexacro.__startCommunication = function (loadItem, path, cachelevel, async, referer, senddata, ndatatype, compress, ver, failpass, service) {
			...
			handlewrap._handle = nexacro.__startLoad(url, bindfn, loadItem.type, servicecachelevel, async, refer_url, senddata, ndatatype, compress, ver, failpass, httpheaderstr);
			return handlewrap;
		};
