...
if (!nexacro._bInitPlatform) {
	...
	_pTouchManager._fling_threshold = 15;
	...
	_pWindow.findComponent = function (elem, x, y) {
		if (x === undefined && y === undefined) {
			...
			if (control_elem) {
				...
				if ((elem instanceof nexacro.ControlElementBase) == false && elem.left == 0 && elem.top == 0) {
	...
	if (!this.application) {
		...
		_pApplication._findTypedefintion = function (typedefintion_url) {
			...
			for (var i = 0; i < _typedef.length; i++) {
		...
		_pApplication.open = function (id, formurl, parent_frame, arr_arg, openstyles, left, top, width, height, opener) {
			...
			var _handle = nexacro._createOpenWindowHandle(parent_window, id, formurl, left, top, width, height, resizable, layered, showontaskbar, false, openstyles);
			if (_handle) {
				return true;
			}
			return false;
