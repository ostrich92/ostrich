...
if (!nexacro.Calendar) {
	...
	_pCalendar.on_change_bindSource = function (propid, ds, row, col, index) {
		if (propid == "value") {
			...
			var add_info = null;
			if (str_val) {
				add_info = this._makeValueStrObj(str_val, true);
				str_val = add_info.full_str;
			}
			var v = this._makeDateObj(str_val, add_info);
	...
	_pCalendar.dropdown = function () {
		...
		var calendaredit = this.calendaredit;
		var add_info = this._makeValueStrObj(calendaredit.text);
		var value = add_info.full_str;
		var post_value = this._makeDateObj(value, add_info);
	...
	_pCalendar.on_notify_ondayclick = function (obj, e) {
		...
		if (e.date instanceof nexacro.Date) {
			...
			var add_info = this._makeValueStrObj(e.date, true);
			post_value = this._makeDateObj(add_info.full_str, add_info);
		}
		var str_preval = pre_value ? pre_value.toString() : pre_value;
		var str_postval = post_value ? post_value.toString() : post_value;
	...
	_pCalendar.on_notify_oneditclick = function (obj, e) {
		if (this.type == "system" && (nexacro.OS == "Android" && nexacro.Browser == "Runtime")) {
	...
	_pCalendar.on_notify_oncloseup = function (obj, e) {
		...
		if (this._day_click) {
			...
			var add_info = this._makeValueStrObj(text);
			var value = add_info.full_str;
			var pre_value = this._makeDateObj(this.value);
			var post_value = this._makeDateObj(value, add_info);
			var str_preval = pre_value ? pre_value.toString() : pre_value;
	...
	_pCalendar._makeSpinValue = function (v, op, pos) {
		var add_info = this._makeValueStrObj(v);
		v = add_info.full_str;
	...
	_pCalendar._makeValueStrObj = function (edit_str, _is_fullvalue) {
		var value_str = "", chkempty_str = false;
		var empty_year = false, empty_mon = false, empty_day = false, empty_hour = false, empty_min = false, empty_sec = false, empty_milesec = false, year_same = true;

		var add_year = "", add_mon = "", add_day = "";
		var editformat_info = this._editformat_info;

		if (editformat_info) {
			if (editformat_info._year || editformat_info._mon || editformat_info._day) {
				if (editformat_info._hour || editformat_info._min || editformat_info._sec || editformat_info._millisec) {
					chkempty_str = true;
					var _info = this._makeFormatInfo("yyyy-MM-dd HH:mm:ss sss");
					value_str = this._makeNormalValue(edit_str);
				}
				else {
					var _info = this._makeFormatInfo("yyyy-MM-dd");
					value_str = this._makeNormalValue1(edit_str);
				}

				if (_is_fullvalue) {
					if (!chkempty_str && !editformat_info._year) {
						empty_year = true;
						add_year = edit_str.substr(0, 4);
						edit_str = edit_str.substr(add_year.length);
					}
					value_str = this._makeMaskValue(edit_str);
				}
			}
			else {
				chkempty_str = true;
				var _info = this._makeFormatInfo("HH:mm:ss sss");
				if (_is_fullvalue) {
					value_str = this._makeMaskValue_timeOnly(edit_str, _info);
				}
				else {
					value_str = this._makeNormalValue(edit_str);
				}
			}
			var valueformat_info = _info;
		}

		var editinfo_list = editformat_info.format_list;
		var valueinfo_list = valueformat_info.format_list;

		var pre_value = this._makeDateObj(this.value);
		var preval_str = pre_value ? pre_value.toString() : pre_value;

		var value_mask = null, edit_mask = null, diff_len = 0, list_idx = 0;
		for (i = 0; i < valueinfo_list.length; i++) {
			value_mask = valueinfo_list[i].mask;
			if (!editinfo_list[list_idx]) {
				if (value_mask.charAt(0) == 'M') {
					empty_mon = true;
					if (empty_year) {
						diff_len = 0;
					}
					else {
						diff_len = year_same ? value_str.length : value_str.length + add_year.length;
					}

					if (_is_fullvalue) {
						add_mon = edit_str.substr(diff_len, 2);
					}
					else {
						add_mon = preval_str.substr(diff_len, 2);
					}
				}
				else if (value_mask.charAt(0) == 'd') {
					empty_day = true;
					diff_len = empty_mon ? diff_len + 2 : value_str.length;

					if (_is_fullvalue) {
						add_day = edit_str.substr(diff_len, 2);
					}
					else {
						add_day = preval_str.substr(diff_len, 2);
					}
				}
				else if (value_mask.charAt(0) == 'H') {
					empty_hour = true;
				}
				else if (value_mask.charAt(0) == 'm') {
					empty_min = true;
				}
				else if (value_mask.charAt(0) == 's') {
					empty_sec = true;
				}
				else if (value_mask == 'sss') {
					empty_milesec = true;
				}
				list_idx++;
			}
			else {
				edit_mask = editinfo_list[list_idx].mask;
				if (value_mask.charAt(0) != edit_mask.charAt(0)) {
					if (!empty_year && value_mask.charAt(0) == 'y') {
						empty_year = true;
						if (_is_fullvalue) {
							add_year = edit_str.substr(0, 4);
						}
						else {
							add_year = preval_str ? preval_str.substr(0, 4) : "0000";
						}
					}
				}
				else {
					if (!empty_year && value_mask.charAt(0) == 'y' && value_mask.length > edit_mask.length) {
						year_same = false;
						if (_is_fullvalue) {
							add_year = edit_str.substr(0, 2);
						}
						else {
							diff_len = value_mask.length - edit_mask.length;
							add_year = preval_str ? preval_str.substr(0, diff_len) : "00";
						}
					}
		...
		var calendaredit = this.calendaredit;
		if (calendaredit && this.type != "monthonly") {
			if (!this._isUndefinedDate(value_str) && !this._isNullDate(value_str) && !this._isEmptyStr(value_str)) {
				if (!year_same || empty_year) {
					if (empty_year) {
						if (chkempty_str) {
							value_str = value_str.substr(add_year.length);
						}
					}
					else {
						value_str = add_year + value_str;
					}
				}
				else {
					add_year = "";
	...
	_pCalendar._makeDateObj = function (v, add_info) {
		if (this._isUndefinedDate(v)) {
			return;
		}
		if (this._isNullDate(v)) {
		...
		if (this.calendaredit && this.type != "monthonly") {
			if (!add_info) {
				var add_info = this._makeValueStrObj(this.calendaredit.text);
			}

			year = (add_info.add_year.length < 4) ? add_info.add_year + year : add_info.add_year;
			mon = (mon && mon.length >= 0) ? mon : add_info.add_mon;
	...
	_pCalendar._setValue = function (v, user_set) {
		...
		if (this._bindsource) {
			...
			if (this._bind_event && this.value && date_obj.valueOf() != this.value.valueOf()) {
				this._currentformat = currentformat;
				return ret;
			}
	...
	_pCalendar._fireKeydownEvent = function () {
		...
		if (edit && edit.value != undefined) {
			...
			if (focus_value != current_value && focus_value != current_value.trim()) {
				...
				prevalue = this._makeDateObj(prevalue);

				var add_info = this._makeValueStrObj(current_value);
				var value = add_info.full_str;
				value = this._makeDateObj(value, add_info);
