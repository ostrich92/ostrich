...
if (!nexacro.ExcelExportObject) {
	...
	_pItem._getCellText = function (source, rowidx, cellidx) {
		...
		if (source && source instanceof nexacro.Grid) {
			...
			if (source.getSubCellCount(band, cellidx)) {
				if (export_obj._is_orgval && display_type == "number") {
	...
	_pItem._getCellBodyStyle = function (cell, idx) {
		...
		if (errstatus < 0) {
			...
			if (is_end) {
				if (is_finish) {
					...
					nexacro._download(excelURL, exportObj._handle, exportfilename_, "undefined", exportObj.exportactivemode);
				}
				else {
		...
		if (bodycntcell > 0) {
			...
			for (var k = 0; k < partRowCount; k++) {
				...
				for (var j = 0; j < bodycntcell; j++) {
					...
					var longdate_flag = false;
					if ((cell_type.indexOf("date") > -1 || cell_type.indexOf("number") > -1) && (this._checkExpr(cell.locale) || this._checkExpr(cell.mask))) {
						sFlag = true;
						if (cell.mask == "LONGDATE") {
							longdate_flag = true;
						}
