	...
	if (!nexacro.Element) {
		...
		_pElement.setElementPosition = function (left, top) {
			if (this.left != left || this.top != top) {
				...
				var _handle = this._handle;
				if (_handle) {
					if (nexacro.OS != "iOS" && this.parent._is_scrollable_comp) {
						nexacro.__setDOMNodeStylePos(_handle.style, 0, 0);
		...
		_pInputElement._on_sys_touchend = function () {
			if (nexacro.OS == "iOS") {
				if (this._keypad_type == "number" && !this._tap_blocked && !this._is_longpressed && !this.readonly) {
		...
		_pScrollableInnerContainerElement.setElementHScrollPos = function (hpos) {
			...
			this.setElementScrollPos(hpos, this._scroll_top);
			return;
		...
		_pPluginElement.callMethod = function () {
			...
			if (this._handle && this._object_node) {
				...
				if (this.plugintype == "application/x-shockwave-flash") {
					if (this._embed_node) {
		...
		_pEventPassOverlayElement.updateCellNodeClient = function (left, top, width, height, cell_hpos, left_padding) {
			var _cell_node = this._cell_node;
			var _image_node = this._image_node;

			if (cell_hpos >= 0 && left_padding >= 0) {
				var right = left + width;

				left = 0;
				left_padding = (left_padding > 0) ? left_padding : 0;

				if (cell_hpos > left_padding) {
			...
			var top_arr = this._getNodeAlignPos();
			if (_cell_node) {
				nexacro.__setDOMNodeStylePos(_cell_node.style, left, top_arr[0]);
				nexacro.__setDOMNodeStyleSize(_cell_node.style, width, height);
			}

			if (_image_node && this.image_url) {
				nexacro.__setDOMNodeStylePos(_image_node.style, left, top_arr[1]);
			}
		};
		...
		_pEventPassOverlayElement.setElementAlignXY = function (halign, valign) {
			...
			var top_arr = this._getNodeAlignPos();
			if (_cell_node) {
				nexacro.__setDOMNodeStylePosLeftTop(_cell_node.style, left, top_arr[0]);
			}

			if (_image_node && this.image_url) {
				nexacro.__setDOMNodeStylePosLeftTop(_image_node.style, left, top_arr[1]);
			}
		};
		...
		_pEventPassOverlayElement._getNodeAlignPos = function () {
			...
			if (_image_node && this.image_url) {
				if (this.height > 0 && this.height > _image_node.height) {
					if (valign == "bottom") {
						image_top = this.height - _image_node.height;
					}
					else if (valign == "middle") {
						image_top = (this.height / 2) - (_image_node.height / 2);
					}
				}

				nexacro.__setDOMNodeStyleAlignXY(_image_node.style, halign, "top");
			}

			return [cell_top, image_top];
		};

		_pEventPassOverlayElement.setElementImageUrl = function (image_url) {
			var _image_node = this._image_node;
			if (_image_node && image_url) {
