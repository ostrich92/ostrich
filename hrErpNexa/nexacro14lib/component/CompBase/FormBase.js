...
if (!nexacro.Form) {
	...
	_pFormBase.addIncludeScript = function (mainurl, url) {
		if (url) {
			...
			for (var i = 0; i < this._includescriptlist.length; i++) {
				if (nexacro._compare(val, this._includescriptlist[i])) {
					return;
				}
			}
			this._includescriptlist.push(val);
		}
	};
	...
	_pFormBase._getTabOrderNext = function (current, direction, bAccessibility, bEditable, edittype, bhotkey, bComposite, bfocus, bNoComposite) {
		if (current && current._is_form && direction > 0 && !bhotkey && bNoComposite) {
	...
	_pFormBase._searchNextComponent = function (current, bSearchFromFirst, opt_force_cycle, bAccessibility, bhotkey) {
		...
		if (my_tabstop_child_cnt > 0 && current && !bSearchFromFirst) {
			...
			if (!next) {
				...
				if (!opt_cycle && (parent._is_frame || this._is_window)) {
					...
					ret = parent._searchNextComponent(this, false, undefined, bAccessibility, bhotkey);
				}
				else {
					next = this._getTabOrderFirst(bAccessibility);
				}
			...
			if (!next) {
				if (this.parent._is_form) {
					ret = this.parent._searchNextComponent(this, undefined, undefined, bAccessibility, bhotkey);
				}
				else {
					ret = null;
				}
		...
		if (next && !ret) {
			...
			if (next._is_form && bAccessibility && next._checkContainerTabFocus() == true) {
				...
				ret = next._searchNextComponent(null, true, undefined, bAccessibility, bhotkey);
			}
			else {
				ret = [next];
			}
	...
	_pForm.on_vscroll = function (obj, e) {
		...
		if (nexacro.OS == "iOS" || (nexacro.Browser == "IE" || nexacro.Browser == "Edge" || nexacro.Browser == "MobileSafari")) {
			var comp = this._last_focused;
	...
	_pForm.getNextComponent = function (comp, no_composite_flag) {
		...
		var next = pThis._getTabOrderNext(comp, 1, false, false, undefined, undefined, undefined, undefined, no_composite_flag);

		if (!next) {
			next = pThis._searchNextComponent(comp, undefined, undefined, false)[0];
		}
