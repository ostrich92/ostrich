...
if (!nexacro.Grid) {
	...
	_pGridExpand.on_find_CurrentStyle_background = function (pseudo) {
		return this._find_pseudo_obj("background", pseudo, "background", true);
	};
	...
	_pGridEdit.on_fire_onchar = function (event_info) {
		if (this._absolutelyValue == true) {
			return true;
		}

		nexacro.EditCtrl.prototype.on_fire_onchar.call(this, event_info);
		return this._grid.on_fire_onchar(this, event_info.chartext, event_info.pretext, event_info.posttext);
	};
	...
	_pGridTextArea.on_fire_onchar = function (event_info) {
		if (this._absolutelyValue == true) {
			return true;
		}

		nexacro.TextAreaCtrl.prototype.on_fire_onchar.call(this, event_info);
		return this._grid.on_fire_onchar(this, event_info.chartext, event_info.pretext, event_info.posttext);
	};
	...
	_pGridMaskEdit.on_fire_onchar = function (event_info) {
		...
		nexacro.MaskEditCtrl.prototype.on_fire_onchar.call(this, event_info);
		return this._grid.on_fire_onchar(this, event_info.chartext, event_info.pretext, event_info.posttext);
	};
	...
	_pGridCalendar.on_fire_onchar = function (obj, chartext, pretext, posttext) {
		...
		return this._grid.on_fire_onchar(obj, chartext, pretext, posttext);
	};
	...
	_pGrid.on_fire_user_ondrag = function (button, alt_key, ctrl_key, shift_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, self_refer_comp) {
		...
		if (!this.selectscrollmode || this.selectscrollmode == "select" || (nexacro._isDesktop() && this.selectscrollmode == "default")) {
			if (evt) {
		...
		if (this._resizer_rowctrl && this._resizer_rowctrl._is_tracking == true) {
			...
			if (evt) {
				...
				this._noFireDragFlag = true;
				ret = [true, this, self_refer_comp, this._getDragData(), null];
			}
		}
		return ret;
	};
	...
	_pGrid._on_move_extratrack = function (obj, windowX, windowY, distX, distY, screenX, screenY) {
		...
		if (rootcomp && this._showEditing == false && (rootcomp != this || 
		...
		if (this.getElement() && this._userRowposChange == false) {
			...
			if (cellNewPos < 0) {
				...
				if (this._is_async_recreate) {
					nexacro.OnceCallbackTimer.callonce(this, proc, 100);
				}
				else {
					proc();
				}
	...
	_pGrid.on_fire_onchar = function (obj, chartext, pretext, posttext) {
	...
	_pGrid._analyzeSuppress = function (exportFlag) {
		...
		for (var j = 0; j < cellcnt; j++) {
			...
			var coltype = ds._getColumnType(cellinfo._col);
			...
			for (var i = 0; i <= total_dispcnt; i++) {
				...
				if (csupp > 0 && i > 0) {
					...
					if ((ds.getRowType(pdatarow) == 16 || ds.getRowType(cdatarow) == 16) && (coltype >= 2 && coltype <= 4)) {
						continue;
					}
	...
	_pGrid._getCellRowTextSize = function (cellinfo, rowidx, text) {
		...
		if (displayType == "image") {
			...
			var controlborder, controlpadding, controllinespace;
			...
			if (usewordwrap && (this._autoSizeRowProc || this._rowSizeEx) && word != "none" && word != false && word != "false") {
				...
				if (editType == "textarea" && this._sizetextarea._input_element) {
					var size2 = [0, 0];
